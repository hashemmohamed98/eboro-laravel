        ...
        <!-- End Navbar -->
            <section class="position-relative main-content sm-content">
                <div class="d-flex align-items-center justify-content-between mb-4">
                    <h2 class="main-color font-bold">{{trans('dashboard.admin.rest_admin')}}</h2>
                    <div class="menu-btn">
                                ...
                                <table class="table table-borderless codiano-datatable table-middle table-hover text-center table-font font-size-14 m-0">
                                    ...
                                    @foreach($Admins as $Admin)
                                        <tr class="border-bottom">
                                            ...
                                            <td class="width-100">
                                                <div>
                                                    <img src="{{asset('/public/uploads/User/'.$Admin->user->image)}}" width="23" alt="">
                                                </div>
                                            </td>
                                            <td class="width-100">
                                            ...
                                            <td class="width-100">
                                                <div>{{$Admin->user->mobile}}</div>
                                            </td>
                                            <td class="width-100">
                                                <div class="d-flex align-items-center justify-content-center">
                                                    <div class="mr-2 cursor-pointer">
                                                        <img src="{{asset('resources/views/dashboard/assets/images/edit.svg')}}" alt="" data-toggle="modal" data-target="#Admin_edit{{$Admin->id}}">
                                                    </div>
                                                    <div class="cursor-pointer">
                                                        <img src="{{asset('resources/views/dashboard/assets/images/delete.svg')}}" alt="" data-toggle="modal" data-target="#delete{{$Admin->user->id}}">
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
...
@foreach($Admins as $Admin)
    ...
    <!-- add admin  -->
    <div class="modal fade" id="Admin_edit{{$Admin->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <h4 class="py-3 px-2"> {{trans('dashboard.admin.admin_info')}} </h4>
                <div class="modal-body">
                    <form action="{{url('api/edit/branch-staff/'. $Admin->id)}}"  class="add_Admin" enctype="multipart/form-data">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="">{{trans('dashboard.cashers.casher_name')}}*</label>
                                <input type="text" name="name" class="form-control radius" placeholder="name" value="{{$Admin->user->name}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="">{{trans('admin.categories.email')}}*</label>
                                <input type="email" name="email" class="form-control radius" placeholder="{{trans('admin.categories.email')}}" value="{{$Admin->user->email}}" >
                            </div>
                            <div class="form-group col-md-6">
                                <label for="">{{trans('dashboard.beverages.branch')}}*</label>
                                <select class="form-control radius" placeholder="branch_id" name="branch_id" required>
                            ...
                            <div class="form-group col-md-6">
                                <label for="">{{trans('admin.phone')}}*</label>
                                <input type="number" name="mobile" class="form-control radius" value="{{$Admin->user->mobile}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="">{{trans('admin.portfolio.address')}}</label>
                                <input type="text" name="address" class="form-control radius" value="{{$Admin->user->address}}">
                            </div>
                            <div class="form-group col-md-6">
                                <label for="">{{trans('admin.password')}}*</label>
                                <input type="password" name="password" class="form-control radius" value="">
                            ...
                            <div class="col-md-6">
                                ...
                                <img src="{{asset('/public/uploads/User/'.$Admin->user->image)}}" width="100" alt="">
                            </div>
                            <div class="col-md-12">
                                <div class="mb-3">
                                    <label for="">{{trans('admin.portfolio.image')}}*</label>
    ...
    <!-- delete modal  -->
    <div class="modal fade" id="delete{{$Admin->user->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{url('api/delete/branch-staff/'.$Admin->id)}}" class="delete">
                    <div class="modal-body">
                        <h6 class="text-dark"> {{trans('admin.categories.delete_p4')}}  {{$Admin->user->name}}</h6>
                    </div>
...
{{-- <button type="button" hidden class="btn btn-primary" id="modalTriger" data-toggle="modal" data-target="#message">
    Launch demo modal
</button>

<div class="modal fade " id="message">
    <div class="modal-dialog w-100 h-100 d-flex align-items-center">

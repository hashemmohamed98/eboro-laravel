    ...
    <main id="resturant-details">
        ...
        <div class="bestinmonth-banner py-5">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card card-box-shadow border-0 border-radius-20 p-4 mb-4">
                            <div class="d-flex align-items-center">
                                <img src="{{asset('/public/uploads/Provider/'.$Providers->logo)}}" class="card-img mx-0 @if($Providers->branch->where('status' , 1)->first() != null)  rating-0 @endif" alt="">
                                <div class="ml-4">
                                    <h4 class="red-color font-size-20 bold">{{$Providers->name}}</h4>
                                    <p class="m-0 font-size-15">Types : @foreach($Providers->typed as $it) {{ $it->type->{'type_'.session('lang')} .' '}}@endforeach</p>

                                    <div class="rating d-flex ml-auto text-left">
                                        {{$avg_rate->value}} <i class="fas fa-star m-1" aria-hidden="true" style="color: #fcd93a"></i>
                                    </div>
                                    <span class="text-muted">{{ $Providers->category->{'name_'.session('lang')} }}</span>
                                    <p class="m-0 font-size-15">{{$Providers->description}}</p>
                                </div>

                            </div>
                            <div class="card-line text-center my-3">
                                <div class="card-poly">
                                    <img src="{{asset('/public/uploads/Provider/'.$Providers->logo)}}" alt="">
                                </div>
                            </div>
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="d-flex align-items-center">
                                    <div class="mr-2">
                                        <i class="fas fa-clock red-color"> </i>{{$Providers->duration}}Min
                                    </div>
                                    @if (Auth::user())
                                        <div id="rating" class="rating d-flex align-items-center" action="{{asset('Rate/'.$Providers->id)}}">
                                ...
                                @if (Auth::user())
                                    <a class="btn red-bg text-white btn-sm"
                                       onclick="event.preventDefault();
                                    document.getElementById('fav').submit();">{{trans('public.add_favourite')}}</a>
                                    <form id="fav" action="{{ asset('Favorite/'.$Providers->id) }}" method="get" style="display: none;">
                                        @csrf
                                    </form>
                                @endif
                            </div>
                                            ...
                                            <div class="grid-content">
                                                ...
                                                <div>
                                                    <a href="{{asset('/product-details/'.$item->id.'/'.$item->name)}}">
                                                        ...
                                                        <div class="c-ele text-muted text-center">{{ $item->typed->{'type_'.session('lang')}??'un-know' }}</div>
                                                        <p class="m-0 font-size-15">{{$item->description}}</p>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="card-line text-center my-3  my-auto">
                                                <div class="card-poly">
                                                    <img src="{{asset('public/uploads/Product/'.$item->image())}}" alt="{{$item->name}}">
                                                </div>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between">
                                                @if($offer = $offers->where('product_id' , $item->id)->first() != null)
